...
export function localizeRichZh(text: string): string {
  ...
  result = result
    ...
    .replace(/^We measure the (.+) only after validating (.+)\.$/g, "我们先验证$2，再测量$1。")
    .replace(/^We solve from measured (.+), then verify on (.+)\.$/g, "我们先根据测得的$1求解，再用$2做验证。")
    .replace(/^We choose the smallest (.+) that still replays (.+)\.$/g, "我们选择仍能回放$2的最小$1。")
    .replace(/^We first validate (.+), then compare (.+)\.$/g, "我们先验证$1，再对比$2。")
    .replace(/^If the task also smells like (.+), say clearly what (.+) adds beyond (.+)\.$/g, "如果这题还带有$1特征，就要明确说清$2相比$3额外带来了什么。")
    .replace(/^Why is the model compact enough\?$/g, "为什么这个模型足够紧凑？")
    .replace(/^How is replay validated\?$/g, "回放是如何验证的？")
    .replace(/^How do you separate DUT distortion from measurement distortion\?$/g, "你们如何区分被测对象失真和测量链路失真？")
    .replace(/^Why can those observables solve the parameter\?$/g, "为什么这些可观测量足以求出目标参数？");

  result = result
    .replace(/\bPeriod-locked\b/gi, "周期锁定")
    .replace(/\bSingle-frequency\b/gi, "单频")
    .replace(/\bdual-channel\b/gi, "双通道")
    .replace(/\bfront end\b/gi, "前端")
    .replace(/\bfront-end\b/gi, "前端")
    .replace(/\banalog\b/gi, "模拟")
    .replace(/\bheadroom\b/gi, "动态余量")
    .replace(/\bfundamental bin\b/gi, "基波谱线")
    .replace(/\bclean sine\b/gi, "纯净正弦")
    .replace(/\bdrive-level\b/gi, "驱动幅度")
    .replace(/\bdrive level\b/gi, "驱动幅度")
    .replace(/\bdrive\b/gi, "驱动")
    .replace(/\bsweep\b/gi, "扫频")
    .replace(/\bexpansion\b/gi, "扩展")
    .replace(/\binjection\b/gi, "注入")
    .replace(/\bpassive parts\b/gi, "无源器件样件")
    .replace(/\bpassive part\b/gi, "无源器件样件")
    .replace(/\bparts\b/gi, "样件")
    .replace(/\bpart\b/gi, "样件")
    .replace(/\bfitting\b/gi, "拟合")
    .replace(/\bfit\b/gi, "拟合")
    .replace(/\bheld-out\b/gi, "留出")
    .replace(/\binstrument\b/gi, "仪器")
    .replace(/\bchain\b/gi, "链路")
    .replace(/\btask\b/gi, "题目")
    .replace(/\btasks\b/gi, "题目")
    .replace(/\bperiod\b/gi, "周期")
    .replace(/\bconfiguration\b/gi, "配置")
    .replace(/\bsetup\b/gi, "配置")
    .replace(/\bindexing\b/gi, "索引")
    .replace(/\border\b/gi, "阶次")
    .replace(/\bcredibility\b/gi, "可信度")
    .replace(/\bexplainability\b/gi, "可解释性")
    .replace(/\bcontaminates\b/gi, "污染了")
    .replace(/\bcontaminate\b/gi, "污染")
    .replace(/\bmonotonic\b/gi, "单调")
    .replace(/\bdominates\b/gi, "主导")
    .replace(/\bdominate\b/gi, "主导")
    .replace(/\breduced\b/gi, "降低")
    .replace(/\bincreased\b/gi, "增加")
    .replace(/\bvaried\b/gi, "变化")
    .replace(/\bunchanged\b/gi, "不变")
    .replace(/\bdistinguishable\b/gi, "可区分")
    .replace(/\binvariant\b/gi, "不变的")
    .replace(/\bablation\b/gi, "消融对比")
    .replace(/\bcomparability\b/gi, "可比性")
    .replace(/\bcoverage\b/gi, "覆盖范围")
    .replace(/\breversible\b/gi, "可逆")
    .replace(/\bmidstream\b/gi, "中途")
    .replace(/\bhonest\b/gi, "可信")
    .replace(/\bcreep in\b/gi, "混入")
    .replace(/\brestore\b/gi, "恢复")
    .replace(/\bback off\b/gi, "降低")
    .replace(/\bclean-source\b/gi, "干净信号源")
    .replace(/\bclean signal source\b/gi, "干净信号源")
    .replace(/\bclean source\b/gi, "干净信号源")
    .replace(/\binjected\b/gi, "注入的")
    .replace(/\bcontrolled\b/gi, "受控")
    .replace(/\bcontrol\b/gi, "控制")
    .replace(/\bmargin\b/gi, "余量")
    .replace(/\banti-aliasing\b/gi, "抗混叠")
    .replace(/\baware\b/gi, "感知")
    .replace(/\btrend\b/gi, "趋势")
    .replace(/\btrends\b/gi, "趋势")
    .replace(/\bnonlinear\b/gi, "非线性")
    .replace(/\bcompensation\b/gi, "补偿")
    .replace(/\badaptation\b/gi, "自适应")
    .replace(/\bwidening\b/gi, "扩展")
    .replace(/\boperating range\b/gi, "工作范围")
    .replace(/\boperating point\b/gi, "工作点")
    .replace(/\bfixed\b/gi, "固定")
    .replace(/\bsimple\b/gi, "简单")
    .replace(/\brobust\b/gi, "稳健")
    .replace(/\breturn\b/gi, "得到")
    .replace(/\bvalues\b/gi, "数值")
    .replace(/\bvisibly\b/gi, "明显地")
    .replace(/\bagreement\b/gi, "一致性")
    .replace(/\bclosest\b/gi, "最接近")
    .replace(/\bamplifier\b/gi, "放大器")
    .replace(/\bmetric\b/gi, "指标")
    .replace(/\bpipeline\b/gi, "处理链")
    .replace(/\blabeling\b/gi, "标签判别")
    .replace(/\blabs\b/gi, "实验题")
    .replace(/\blab\b/gi, "实验")
    .replace(/\blinearity\b/gi, "线性度")
    .replace(/\bclipping\b/gi, "削顶")
    .replace(/\bleakage\b/gi, "泄漏")
    .replace(/\bbias\b/gi, "偏置")
    .replace(/\btrend\b/gi, "趋势")
    .replace(/\bsettings\b/gi, "配置")
    .replace(/\bsetting\b/gi, "配置")
    .replace(/\bfixed\b/gi, "固定")
    .replace(/\bpoint\b/gi, "工作点")
    .replace(/\bbin\b/gi, "谱线")
    .replace(/\bsine\b/gi, "正弦")
    .replace(/\bsource\b/gi, "信号源")
    .replace(/\bsources\b/gi, "信号源")
    .replace(/\bstory\b/gi, "结论链路")
    .replace(/\blayer\b/gi, "层")
    .replace(/\bbaseline\b/gi, "基线")
    .replace(/\bfloor\b/gi, "底噪")
    .replace(/\bcompare\b/gi, "对比")
    .replace(/\bcomparison\b/gi, "对比")
    .replace(/\bversus\b/gi, "相对")
    .replace(/\bcase\b/gi, "样例")
    .replace(/\bcases\b/gi, "样例")
    .replace(/\bop-amp\b/gi, "运放")
    .replace(/\btriggered\b/gi, "触发式")
    .replace(/\bmirror\b/gi, "镜像")
    .replace(/\btransport\b/gi, "传输")
    .replace(/\bwirelessly\b/gi, "通过无线链路")
    .replace(/\bwireless\b/gi, "无线")
    .replace(/\bidentity-preserving\b/gi, "保留身份一致性的")
    .replace(/\btime base\b/gi, "时基")
    .replace(/\brms\b/gi, "RMS")
    .replace(/\bfft\b/gi, "FFT")
    .replace(/\bthd\b/gi, "THD")
    .replace(/\brlc\b/gi, "RLC")
    .replace(/\badc\b/gi, "ADC")
    .replace(/\bdac\b/gi, "DAC")
    ...
    .replace(/\band\b/gi, "与")
    .replace(/\bor\b/gi, "或")
    .replace(/\bthen\b/gi, "再")
    .replace(/\bbefore\b/gi, "之前")
    .replace(/\bafter\b/gi, "之后")
    .replace(/\bunder\b/gi, "在")
    .replace(/\bwhile\b/gi, "在")
    .replace(/\bwhen\b/gi, "当")
    .replace(/\bwhere\b/gi, "在")
    .replace(/\bwith\b/gi, "配合")
    .replace(/\bwithout\b/gi, "不带")
    .replace(/\bfor\b/gi, "用于")
    .replace(/\bfrom\b/gi, "从")
    .replace(/\binto\b/gi, "得到")
    .replace(/\bplus\b/gi, "加")
    .replace(/\bkept\b/gi, "保持")
    .replace(/\bkeep\b/gi, "保持")
    .replace(/\bshow\b/gi, "展示")
    .replace(/\bfreeze\b/gi, "冻结")
    .replace(/\brequires\b/gi, "需要")
    .replace(/\brequire\b/gi, "需要")
    .replace(/\benough\b/gi, "足够的")
    .replace(/\bthe\b/gi, "")
    .replace(/\ba\b/gi, "")
    .replace(/\ban\b/gi, "")
    .replace(/\bis\b/gi, "是")
    ...
    .replace(/\bstill\b/gi, "仍然")
    .replace(/\bsurvives\b/gi, "保持有效")
    .replace(/\badded\b/gi, "加入")
    .replace(/\badd\b/gi, "增加");
